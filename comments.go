...
func NewReader(r io.Reader) io.Reader { return &reader{r, fstate(text), defaultDelim} }

// NewCustomReader is identical to NewReader, except that it accepts a custom
// delimeter
func NewCustomReader(r io.Reader, delim byte) io.Reader { return &reader{r, fstate(text), delim} }
...
func (r *reader) Read(buf []byte) (int, error) {
	...
	for rcount != len(buf) {
		...
		written, read, r.state = r.state.run(buf[wcount:], buf[rcount:], r.delim)
		wcount += written
		rcount += read
	}
	return wcount, err
...
type state interface {
	run(dst, src []byte, delim byte) (written, read int, next state)
}

type fstate func(dst, src []byte, delim byte) (written, read int, next state)
func (f fstate) run(dst, src []byte, delim byte) (written, read int, next state) { return f(dst, src, delim) }
